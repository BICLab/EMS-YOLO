...
def run(data,
    ...
    if not training:
        ...
        if pt and device.type != 'cpu':
            model(torch.zeros(1, 5,3, imgsz, imgsz).to(device).type_as(next(model.model.parameters())))  # warmup
        pad = 0.0 if task == 'speed' else 0.5
        task = task if task in ('train', 'val', 'test') else 'test'  # path to train/val/test images
        dataloader = create_dataloader('path', 250000,T,image_shape,task,batch_size, stride, single_cls, pad=pad, rect=pt,
    ...
    sz=np.array(SZ)
    np.save('firerate_exp30.npy',firerate)
    np.save('size_exp30.npy',sz)
    # Compute metrics
